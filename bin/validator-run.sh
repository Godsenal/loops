#!/bin/zsh
# 검증자 탭 본체: 검증 전용 worktree(cwd)에서 headless claude로 human-gate 제안을 심문하고, 판정을 남긴 뒤 worktree를 스스로 걷는다.
# 발굴자/검증자 분리의 checker 쪽 — Edit/Write/NotebookEdit를 **--disallowedTools로 구조적으로 차단**해
# "검증자가 코드를 고치는" 사고를 프롬프트가 아니라 도구 부재로 막는다(verifier·봇 에이전트와 동일 원칙).
set -u
